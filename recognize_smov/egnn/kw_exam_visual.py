...
def plot(img_path, json_path_cz,json_path_jj, kw_labels, target_folder, psize=35):
    img = cv2.imread(img_path, 0)
    points, edge_index, cz_labels, _ = load_data_cz(json_path_cz, max_edge_lengh=28)
    points, edge_index, jj_labels, _ = load_data_jj(json_path_jj, max_edge_lengh=28)
    ...
    mask_pd_kw = np.zeros(img.shape)
    mask_pd_kw[points[:, 0], points[:, 1]] = kw_labels
    mask_pd_kw = np.array(mask_pd_kw, np.uint8)
    h, w = np.where(mask_pd_kw == 3)
    plt.scatter(w, h, s=psize, c='green')
...
if __name__ == '__main__':
    ...
    for json_path in (json_lst):
        ...
        jj_json_path = json_path.replace('smov', 'gb')
        cz_json_path = json_path.replace('smov', 'dophant_atom')

        target_folder = '../../gb+smov_visual/'
        if not os.path.exists(target_folder):
            os.makedirs(target_folder)

        plot(img_path, cz_json_path, jj_json_path, labels, target_folder)

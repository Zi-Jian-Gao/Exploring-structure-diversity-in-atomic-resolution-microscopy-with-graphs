...
def get_training_augmentation():
    train_transform = [
        ...
        # A.RandomBrightnessContrast(brightness_limit=(-0.05, 0.), contrast_limit=(-0.1, 0.), p=0.8),
        # A.RandomBrightnessContrast(brightness_limit=(-0.05, 0.05), contrast_limit=(-0.05, 0.05), p=0.8),
        # A.RandomBrightnessContrast(brightness_limit=(-0.2, 0.), contrast_limit=(-0.2, 0.), p=0.8),
        # A.RandomBrightnessContrast(brightness_limit=(-0.3, 0.), contrast_limit=(-0.3, 0.), p=0.8),
        # A.Normalize(),
    ]
...
def load_data(json_path):
    ...
    labels = np.array([int(np.max(light) > 90) + int(np.max(light) > 160) for light in lights])
...
class AtomDataset_v2_aug(InMemoryDataset):
    ...
    def process(self):
        ...
        print('Root:', self.root, 'Json data number: {}.'.format(len(json_lst)))
        repeat_numbers = 48 if 'train' in self.root else 1
        aug = get_training_augmentation() if 'train' in self.root else get_validation_augmentation()

        data_lst = []
        for i in range(repeat_numbers):

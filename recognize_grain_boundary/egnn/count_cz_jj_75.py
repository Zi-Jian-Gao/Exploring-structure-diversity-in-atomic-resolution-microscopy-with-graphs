...
def count( json_path_cz, jj_labels,point_lst_dict, psize=35):
    ...
    if np.size(AA) == 0:
        ...
        for point in points:
            ...
            # 检查8个原子的列表是否存在
            if 7 in point_lst_dict:
                # 如果存在，检查坐标是否属于8个原子的列表
                for atom_point_lst in point_lst_dict[7]:
            ...
            if 5 in point_lst_dict and continue_outer_loop:
                for atom_point_lst in point_lst_dict[5]:
                    if any(np.allclose(point, atom_point) for atom_point in atom_point_lst):
                        count_4_atoms += 1
                        continue_outer_loop = False  # 找到匹配后，设置标志变量为False，以退出外层循环
                        break  # 退出当前的内层循环
        ...
        print(f"属于7个原子数量的list的原子数量: {count_8_atoms}")
        print(f"属于5个原子数量的list的原子数量: {count_4_atoms}")
...
if __name__ == '__main__':
    ...
    print(f"所有属于7个原子数量的list的原子数量: {total_count_8_atoms}")
    print(f"所有属于5个原子数量的list的原子数量: {total_count_4_atoms}")
